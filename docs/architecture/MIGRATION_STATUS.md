...
- Added [index.py](/Users/luisencinas/Documents/GitHub/brain-ops/src/brain_ops/domains/knowledge/index.py) for entity index generation: scans vault for entity notes, groups them by type, renders a navigable INDEX markdown with wikilinks and per-type counts
- Added [relations.py](/Users/luisencinas/Documents/GitHub/brain-ops/src/brain_ops/domains/knowledge/relations.py) for entity relationship extraction: extracts typed relations from frontmatter `related` fields, builds bidirectional adjacency maps, finds all connections for a given entity, renders relationship markdown with wikilinks
